...
from edge_ai.config import PROJECT_ROOT, CheckerboardConfig

FIXED_TIME = datetime(2026, 7, 30, 1, 2, 3, tzinfo=timezone.utc)


def _temporary_directory() -> TemporaryDirectory[str]:
    return TemporaryDirectory(dir=PROJECT_ROOT / ".pytest_cache")
...
def test_target_write_refuses_overwrite_and_writes_metadata() -> None:
...
def test_target_write_requires_png_extension() -> None:
    with _temporary_directory() as temporary_directory:
...
def test_detects_front_facing_board_with_refined_finite_corners() -> None:
    with _temporary_directory() as temporary_directory:
...
def test_detects_controlled_perspective_board() -> None:
    with _temporary_directory() as temporary_directory:
...
def test_limited_coverage_is_a_warning_not_rejection() -> None:
    with _temporary_directory() as temporary_directory:
...
def test_detection_failure_on_blank_image() -> None:
    with _temporary_directory() as temporary_directory:
...
def test_unreadable_image_handling() -> None:
    with _temporary_directory() as temporary_directory:
...
def test_incorrect_resolution_is_rejected() -> None:
    with _temporary_directory() as temporary_directory:
        image_path = Path(temporary_directory) / "small.png"
        assert cv2.imwrite(
