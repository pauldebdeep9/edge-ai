...
def test_dataset_manifest_order_counts_warnings_and_annotations(
    ...
    with TemporaryDirectory(dir=temporary_root) as temporary_directory:
        ...
        annotated_directory = root / "annotated"
        ...
        manifest = inspect_dataset.inspect_dataset(
            _options(
                input_directory, annotated_directory, manifest_path
            ),
...
def test_dataset_enumeration_is_deterministic() -> None:
    temporary_root = PROJECT_ROOT / ".pytest_cache"
    with TemporaryDirectory(dir=temporary_root) as temporary_directory:
...
def test_dataset_outputs_refuse_overwrite() -> None:
    temporary_root = PROJECT_ROOT / ".pytest_cache"
    with TemporaryDirectory(dir=temporary_root) as temporary_directory:
        root = Path(temporary_directory)
        input_directory = root / "input"
        ...
        with pytest.raises(
            ...
            inspect_dataset.inspect_dataset(
                options, generated_at=FIXED_TIME
            )


def test_dataset_overwrite_is_explicitly_allowed() -> None:
    temporary_root = PROJECT_ROOT / ".pytest_cache"
    with TemporaryDirectory(dir=temporary_root) as temporary_directory:
        root = Path(temporary_directory)
        input_directory = root / "input"
