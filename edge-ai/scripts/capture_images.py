...
@dataclass(frozen=True)
class DeliveredFpsMeasurement:
    """Measured frame delivery over a bounded headless smoke test."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "fail if MJPG was requested but the camera did not negotiate MJPG"
        ),
    )
    return parser
...
def parse_options(argv: Sequence[str] | None = None) -> CaptureOptions:
    ...
    return CaptureOptions(
        ...
        strict_mjpeg=arguments.strict_mjpeg,
    )
...
def measure_delivered_frame_rate(
    camera: CameraContext,
    frame_count: int,
    *,
    monotonic_provider: MonotonicProvider = perf_counter,
) -> DeliveredFpsMeasurement:
    """Read frames and measure delivered FPS with a monotonic timer."""
    if frame_count <= 0:
        raise ValueError("frame_count must be positive")
    started_at = monotonic_provider()
    for _ in range(frame_count):
        camera.read()
    elapsed_seconds = monotonic_provider() - started_at
    delivered_fps = (
        frame_count / elapsed_seconds
        if elapsed_seconds > 0.0
        else float("inf")
    )
    return DeliveredFpsMeasurement(
...
def require_mjpeg_if_strict(report: CameraReport) -> None:
    """Fail strict smoke tests when requested MJPG was not negotiated."""
    if (
        report.requested_fourcc == "MJPG"
        and report.negotiated_fourcc != "MJPG"
    ):
        raise CaptureWorkflowError(
            "MJPG was requested but not negotiated; "
            f"camera reported {report.negotiated_fourcc}"
        )
...
def run_headless_smoke_test(
    ...
    *,
    ...
    measurement = measure_delivered_frame_rate(
        camera, frame_count, monotonic_provider=monotonic_provider
    )
    report = camera.report
    print(f"Headless smoke test frames read: {measurement.frame_count}")
    print(f"FPS requested: {report.requested_fps:g}")
    print(f"FPS driver-reported: {report.negotiated_fps:g}")
    print(
        "FPS measured delivered: "
        f"{measurement.delivered_fps:.2f} "
        f"over {measurement.elapsed_seconds:.3f}s"
    )
    ...
    *,
    ...
    try:
        ...
        with camera_context as camera:
            ...
            if options.strict_mjpeg:
                require_mjpeg_if_strict(camera.report)
