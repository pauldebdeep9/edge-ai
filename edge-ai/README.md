...
Nano with a Logitech C920 webcam.
...
- software-only environment/configuration inspection;
- portable camera-opening and capture workflow with backend fallback;
- headless C920 smoke-test path that reports requested and negotiated settings;
- checkerboard target generation;
- offline checkerboard dataset inspection and annotated previews;
- offline intrinsic-calibration mathematics from a Task 4 manifest;
- offline undistortion validation from saved intrinsic-calibration output.

Still pending:

- real NVIDIA Jetson Orin Nano hardware validation with the Logitech C920;
- real C920 intrinsic calibration from captured, diverse checkerboard images;
- metric distance measurement;
- YOLO integration.

The offline tests do not use a camera. Do not treat successful software tests
or Windows webcam checks as Jetson validation.
...
To make MJPG negotiation mandatory during a smoke test, add
`--strict-mjpeg`. Without that flag, the smoke test reports requested and
negotiated FOURCC values but does not fail solely because a different FOURCC
was negotiated.
...
treated as a material negotiation failure. The headless smoke test also
reports delivered FPS measured over the requested smoke-test frames with a
monotonic timer. Offline tests avoid timing assertions on that measured value.
...
Jetson Orin Nano and C920 hardware validation remains pending until these
commands run on the Jetson.
...
The intrinsic-calibration software path is implemented and synthetically
tested, but real C920 intrinsic calibration remains pending until a real
checkerboard dataset is captured and calibrated.
