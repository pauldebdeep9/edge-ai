...
def test_valid_manifest_loading_preserves_warnings_and_sorts_names(
    tmp_path: Path,
) -> None:
...
def test_insufficient_accepted_views(tmp_path: Path) -> None:
    path = tmp_path / "manifest.json"
...
def test_successful_synthetic_calibration_and_error_counts(
    tmp_path: Path,
) -> None:
...
def test_controlled_noise_increases_reprojection_error(tmp_path: Path) -> None:
    clean = _calibrate_from_payload(
        _manifest_payload(), tmp_path / "clean.json"
    )
    noisy = _calibrate_from_payload(
        _manifest_payload(noise_sigma=0.3), tmp_path / "noisy.json"
    )
...
def test_cli_success_for_deterministic_synthetic_input(
    ...
    assert (tmp_path / "results" / "synthetic.npz").exists()
    assert (tmp_path / "results" / "synthetic.yaml").exists()
