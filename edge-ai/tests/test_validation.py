...
def test_loads_valid_npz_without_pickle(tmp_path: Path) -> None:
...
def test_loads_valid_opencv_yaml(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
...
def test_rejects_malformed_npz(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "calibration.npz"
...
def test_rejects_unsupported_calibration_extension(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
...
def test_rejects_missing_calibration_file(tmp_path: Path) -> None:
    missing = tmp_path / "missing-calibration.npz"

    with pytest.raises(CalibrationLoadError, match="does not exist"):
        load_calibration(missing)


def test_rejects_invalid_camera_matrix_shape(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "calibration.npz"
        _write_npz(
            path,
...
def test_rejects_missing_distortion_coefficients(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "calibration.npz"
...
def test_loads_readable_image_without_changing_it(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
...
def test_rejects_unsupported_image_extension(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
...
def test_rejects_empty_image_array(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "image.png"
        path.touch()
...
def test_exact_resolution_matches_and_mismatch_is_rejected(
    tmp_path: Path,
) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "image.png"
...
def test_undistortion_alpha_endpoints_and_dimensions(
    alpha: float, tmp_path: Path
) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "image.png"
        _write_image(path)
...
def test_rejects_invalid_alpha(alpha: float, tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "image.png"
        _write_image(path)
        source = load_validation_image(path)
...
def test_no_crop_option_omits_valid_roi_crop(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "image.png"
        _write_image(path)
        source = load_validation_image(path)
...
def test_radial_undistortion_is_deterministic_and_changes_mapping(
    tmp_path: Path,
) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        path = Path(temporary_directory) / "image.png"
...
def test_persistence_writes_reloadable_lossless_set_and_preserves_source(
    tmp_path: Path,
) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
...
def test_persistence_refuses_overwrite(tmp_path: Path) -> None:
    with _temporary_directory(tmp_path) as temporary_directory:
        root = Path(temporary_directory)
        image_path = root / "source.png"
        _write_image(image_path)
...
def test_persistence_failure_cleans_partial_output_set(
    tmp_path: Path,
) -> None:
    calls = 0
